...
public class ATKPolygonView {
	...
	public float getAcres(){		
		if(this.polygon.boundary.size() < 3) return 0.00f;
		
		double earth_radius = (double) 6378137.0f; // in meters  originally 6371009.0f
		double lat_dist = (double) ((Math.PI * earth_radius) / (double) 180.00f);
		...
		newArea = Math.abs(newArea) / (2.00f * 4046.68f);
		//todo formula for hectares

		//newArea = newArea/2.471f;
